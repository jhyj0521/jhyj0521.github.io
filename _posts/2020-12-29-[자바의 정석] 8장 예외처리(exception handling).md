...
*입출력에 사용되는 클래스 중에서는 사용했던 자원을 반환하기 위해 사용한 후 꼭 닫아 줘야 하는 것들이 있다.*
```java
try {
...
*한 메서드에서 발생할 수 있는 예외가 여럿인 경우, 심지어는 단 하나의 예외에 대해서도 예외가 발생한 메서드와 호출한 메서드 양쪽에서 처리하도록 할 수 있다.*  
...
* 원인 예외로 등록해서 다시 예외를 발생시키는 이유
	...
	* checked예외를 unchecked예외로 바꿈으로서 예외처리를 선택적으로 만들어 억지로 예외처리를 하지 않게 하기 위해  
